        ...
        <div className='Sport'>
            {window.location.pathname === "/sport/cordage" && 
                <nav className='Sport-body'>
                    {CORDAGE.map((item) => {
                        return(
                            <ul style={{backgroundColor: (item.id === 0 || item.id === 26 || item.id === 29) && "#f89393"}} key={item.id} className={Cordage === item.id ? "Sport-select" : ""} onClick={() => editCordage(item)}>
            ...
            {window.location.pathname === "/sport/kungfu" && 
                <nav className='Sport-body'>
                    {BODY.map((item) => {
                        return(
                            <ul style={{backgroundColor: (item.id === 0 || item.id === 30 || item.id === 41) && "#f89393"}} key={item.id} className={Body === item.id ? "Sport-select" : ""} onClick={() => editBody(item)}>
                                <li>{item.id + 1}. {item.name}</li>
                                {item.quantity ? <span>{item.quantity}</span> : <span>{item.duration}</span>}
                            </ul>
                        )
